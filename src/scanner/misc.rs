...
use input::ScanInput;
use util::StrUtil;
use super::{ScanFromStr, ScanSelfFromStr, ScanFromBinary, ScanFromOctal, ScanFromHex};
...
fn test_binary() {
    ...
    assert_match!(Binary::<i32>::scan_from("110010101110000b"),
                  Ok((0x6570, 15)));
}
...
fn test_everything() {
    ...
    assert_match!(Everything::<&str>::scan_from("うまいー　うまいー　ぼうぼうぼうぼう"),
                  Ok(("うまいー　うまいー　ぼうぼうぼうぼう", 54)));
}
...
fn test_hor_space() {
    ...
    assert_match!(HorSpace::<&str>::scan_from(""), Err());
    assert_match!(HorSpace::<&str>::scan_from("a"), Err());
    assert_match!(HorSpace::<&str>::scan_from("0"), Err());
    assert_match!(HorSpace::<&str>::scan_from(" "), Ok((" ", 1)));
    assert_match!(HorSpace::<&str>::scan_from("\t"), Ok(("\t", 1)));
    assert_match!(HorSpace::<&str>::scan_from("\r"), Err());
...
fn match_ident(s: &str) -> Option<usize> {
    ...
    let len = ics.take_while(|&(_, c)| XID_Continue_table.span_table_contains(&c))
                 .map(|(i, c)| i + c.len_utf8())
                 .last()
                 .unwrap_or(first_len);
...
fn test_ident() {
    ...
    assert_match!(Ident::<&str>::scan_from(""), Err());
    ...
    assert_match!(Ident::<&str>::scan_from("0123abc456 "), Err());
    assert_match!(Ident::<&str>::scan_from("_0123abc456 "),
                  Ok(("_0123abc456", 11)));
...
fn test_newline() {
    ...
    assert_match!(Newline::<&str>::scan_from(""), Err());
    assert_match!(Newline::<&str>::scan_from("x"), Err());
...
fn test_non_space() {
    ...
    assert_match!(NonSpace::<&str>::scan_from(""), Err());
    assert_match!(NonSpace::<&str>::scan_from(" abc "), Err());
    ...
    assert_match!(NonSpace::<&str>::scan_from("abc\u{200B}"),
                  Ok(("abc\u{200b}", 6)));
...
fn test_number() {
    ...
    assert_match!(Number::<&str>::scan_from(""), Err());
    assert_match!(Number::<&str>::scan_from("a"), Err());
    assert_match!(Number::<&str>::scan_from("0"), Ok(("0", 1)));
    assert_match!(Number::<&str>::scan_from("0x"), Ok(("0", 1)));
    assert_match!(Number::<&str>::scan_from("x0"), Err());
    assert_match!(Number::<&str>::scan_from("123 456 xyz"), Ok(("123", 3)));
    assert_match!(Number::<&str>::scan_from("123 456 xyz"), Ok(("123", 3)));
    assert_match!(Number::<&str>::scan_from("123４５６789 "),
                  Ok(("123４５６789", 15)));
    assert_match!(Number::<&str>::scan_from("𐒩０꘠᧑ "),
                  Ok(("𐒩０꘠᧑", 13)));
}
...
fn test_quoted_string() {
    ...
    assert_match!(QS::scan_from(""), Err());
    assert_match!(QS::scan_from("dummy xyz"), Err());
    assert_match!(QS::scan_from("'dummy' xyz"), Err());
...
fn test_space() {
    ...
    assert_match!(Space::<&str>::scan_from(""), Err());
    assert_match!(Space::<&str>::scan_from("a"), Err());
    assert_match!(Space::<&str>::scan_from("0"), Err());
...
fn test_word() {
    ...
    assert_match!(Word::<&str>::scan_from(""), Err());
    ...
    assert_match!(Word::<&str>::scan_from("123 456 xyz"), Ok(("123", 3)));
    assert_match!(Word::<&str>::scan_from("123４５６789 "),
                  Ok(("123４５６789", 15)));
    assert_match!(Word::<&str>::scan_from("𐒩０꘠᧑ "),
                  Ok(("𐒩０꘠᧑", 13)));
    assert_match!(Word::<&str>::scan_from("kumquat,bingo"), Ok(("kumquat", 7)));
    assert_match!(Word::<&str>::scan_from("mixed言葉كتابة "),
                  Ok(("mixed言葉كتابة", 21)));
}
